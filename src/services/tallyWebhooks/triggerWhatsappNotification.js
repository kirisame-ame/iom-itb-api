...
async function getTemplateByForm(formSlug) {
  const key = FORM_SLUG_TO_TEMPLATE_KEY[formSlug];
  ...
  if (key) {
    try {
      ...
      console.warn(
        "[WhatsApp] DB template lookup failed, falling back:",
        err.message,
      );
    }
  }

  // Env var fallback (kept for backward compatibility)
  const perFormEnv = {
    pendaftaran_anggota:
...
const triggerWhatsappNotification = async ({
  ...
  const template = await getTemplateByForm(formSlug);
